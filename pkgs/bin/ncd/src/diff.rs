...
use pathfinding::{kuhn_munkres, matrix::Matrix};
use size::Size;
...
use crate::{
    ...
    version::{VersionComponent, VersionPiece},
};

fn create_backend<'a>(force_correctness: bool) -> store::CombinedStoreBackend<'a> {
...
impl DerivationSelectionStatus {
    fn from_names(name: &str, old: &HashSet<String>, new: &HashSet<String>) -> Self {
...
pub fn write_package_diff(
    ...
    let paths_old = connection
        ...
        .with_context(|| format!("failed to query dependencies of '{}'", path_old.display()))?;

    // Query dependencies for new path
    let paths_new = connection
        .query_dependents(path_new)
        .with_context(|| format!("failed to query dependencies of '{}'", path_new.display()))?;

    // Query system derivations for old path
    let system_derivations_old =
        connection
...
pub fn match_version_lists<'a>(
    ...
    for i in 0..from.len() {
        for j in 0..to.len() {
            distances[(i, j)] = i32::try_from(levenshtein(&from_components[i], &to_components[j]))
                .unwrap_or_else(|err| {
    ...
    let (_cost, matchings) = kuhn_munkres::kuhn_munkres_min::<i32, Matrix<i32>>(&distances);

    // Process matched pairs
    let mut remaining = (0..to.len()).collect::<HashSet<usize>>();
    let mut pairings = Vec::<EitherOrBoth<&Version>>::with_capacity(from.len() + to.len());
...
fn collect_system_names(paths: impl Iterator<Item = StorePath>, context: &str) -> HashSet<String> {
    paths
        .filter_map(|path| match path.parse_name_and_version() {
         ...
         paths: &mut HashMap<String, (Vec<Version>, Vec<Version>)>| {
            match path.parse_name_and_version() {
                Ok((name, version)) => {
                    ...
                    list.push(version.unwrap_or_else(|| Version::from("<none>".to_owned())));
                }
...
fn render_diffs(
    ...
    for diff in diffs {
        ...
        let (old_str, new_str) = fmt_version_diffs(&diff.old, &diff.new, diff.has_common_versions)?;
...
pub fn generate_diffs_from_paths<S: BuildHasher>(
    ...
    for (name, (mut old_versions, mut new_versions)) in paths {
        ...
        let status = match (common_count, unique_old.is_empty(), unique_new.is_empty()) {
            (_, true, true) => continue,
            (0, true, false) => DiffStatus::Added,
            (0, false, true) => DiffStatus::Removed,
            (_, true, false) | (_, false, true) => DiffStatus::Changed(Change::UpgradeDowngrade),
            (_, false, false) => {
                if let Some(status) = determine_change_status(&unique_old, &unique_new) {
                    status
                } else {
                    continue;
                }
...
pub fn add_selection_status(
    ...
    for diff in diffs {
        diff.selection =
            DerivationSelectionStatus::from_names(&diff.name, system_paths_old, system_paths_new);
    }
}
...
mod tests {
    ...
    use crate::{
        diff::{
            Change, DerivationSelectionStatus, Diff, DiffStatus, add_selection_status, levenshtein,
            match_version_lists,
        },
        generate_diffs_from_paths,
        version::{Version, VersionComponent, VersionPiece},
    };
    ...
    fn generate_diffs_from_paths_test() {
        ...
        let diff_1 = (
            vec![Version::new("1.1.0"), Version::new("1.3")],
            vec![Version::new("1.1.0"), Version::new("1.4")],
        );
        ...
        let diff_2 = (
            vec![Version::new("1.2.0"), Version::new("1.5")],
            vec![Version::new("1.2.0")],
        );
