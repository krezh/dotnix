...
impl StorePath {
    ...
    fn parse_name_and_version(&self) -> Result<(&str, Option<Version>)> {
        ...
        let captures = STORE_PATH_REGEX
            .captures(path)
            .ok_or_else(|| anyhow!("path '{path}' does not match expected Nix store format"))?;
        ...
        let version: Option<Version> = captures
            .get(2)
            .map(|capture| Version::from(capture.as_str().trim_start_matches('-').to_owned()));
