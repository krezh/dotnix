...
use crate::{path_to_canonical_string, store::queries};
...
pub(crate) fn default_close_inner_connection(
    ...
    let conn = maybe_conn
        .take()
        .ok_or_else(|| anyhow!("Tried to close connection to {} that does not exist", path))?;
...
pub(crate) fn query_closure_size(conn: &Connection, path: &Path) -> Result<Size> {
