...
use std::{fmt::Display, iter::Iterator, path::Path};
...
use crate::{
    StorePath,
    store::{db_eager::EagerDBConnection, nix_command::CommandBackend, store::LazyDBConnection},
};
...
pub const DATABASE_PATH_IMMUTABLE: &str = "file:/nix/var/nix/db/db.sqlite?immutable=1";
...
pub trait StoreBackend<'a> {
    ...
    fn query_dependents(&self, path: &Path) -> Result<Box<dyn Iterator<Item = StorePath> + '_>>;
}
        ...
        F: Fn(&'b Box<dyn StoreBackendPrintable<'a>>, &Path) -> Result<Ret>,
    {
        ...
        for (i, backend) in self.backends.iter().enumerate() {
            if !backend.connected() {
                warn!("Skipping backend {i} ({backend}) in query {path:?}: not connected");
                continue;
            }
...
impl<'a> StoreBackend<'a> for CombinedStoreBackend<'a> {
    ...
    fn connect(&mut self) -> Result<()> {
        ...
        if any_succeeded {
            ...
            combined_err = combined_err.map(|err| err.context("All backends failed to connect."));
