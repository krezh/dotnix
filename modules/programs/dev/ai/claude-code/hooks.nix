{ ... }:
{
  flake.modules.homeManager.ai =
    { pkgs, lib, ... }:
    let
      # Equivalent of `herdr integration install claude`, reproduced declaratively.
      # Reports the Claude Code session id/transcript path to herdr's local socket on
      # SessionStart so herdr can track/restore the pane; no-ops unless the session is
      # actually running inside a herdr-managed pane (HERDR_ENV/HERDR_SOCKET_PATH/HERDR_PANE_ID).
      herdrAgentStateHook = pkgs.writeShellScript "herdr-agent-state" ''
        set -eu
        ...
        HERDR_ACTION="$action" HERDR_HOOK_INPUT_FILE="$hook_input_file" ${lib.getExe pkgs.python3} - <<'PY'
        import json
        import os
        import random
        import socket
